...
- [ ] **파일**: `src/app/profile/page.tsx`
  - [ ] `analysis_count === 0` 조건 추가
  - [ ] "분석가능 횟수" 카드 내에 조건부 버튼 렌더링
  - [ ] 버튼 클릭 시 `/recharge` 페이지로 라우팅
  - [ ] 버튼 UI: 파란색 계열, "분석가능횟수 충전" 텍스트

---

### 3. Recharge 페이지 생성
- [ ] **파일**: `src/app/recharge/page.tsx` (신규 생성)
  - [ ] 페이지 레이아웃 구성
  - [ ] 충전 플랜 카드 2개 구성:
    - **플랜 1**: 20회 - 500원
    - **플랜 2**: 50회 - 1000원
  - [ ] 각 플랜 선택 시 토스 페이먼츠 결제 페이지로 이동
  - [ ] 토스 페이먼츠 위젯 초기화 로직 작성
  - [ ] 결제 요청 시 필요한 정보: 금액(amount), 주문명(orderName), 주문 ID(orderId)
...
- [ ] **파일**: `src/app/recharge/success/page.tsx` (신규 생성)
  - [ ] 결제 성공 시 토스로부터 받은 쿼리 파라미터 파싱
  - [ ] `/api/payment/confirm` API 호출하여 결제 승인 요청
  - [ ] 성공 시 사용자 분석 횟수 증가
  - [ ] 성공 메시지 표시 후 `/profile` 페이지로 리디렉션

- [ ] **파일**: `src/app/recharge/fail/page.tsx` (신규 생성)
  - [ ] 결제 실패 시 오류 메시지 표시
  - [ ] "다시 시도" 버튼으로 `/recharge` 페이지로 돌아가기
